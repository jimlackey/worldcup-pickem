...
import { updateTeamAction, type CountryActionResult } from "./actions";
...
interface CountryManagerProps {
  ...
  poolSlug: string;
  /**
   * When true, hide all edit affordances — rows render as static info only.
   * Used for real (non-demo) pools that share global team data; edits for
   * those must go through /super-admin/countries.
   */
  readOnly?: boolean;
}
...
export function CountryManager({
  ...
  // Track which group is expanded. Default to the first group so the admin
  // immediately sees the UI rather than a wall of collapsed headers.
...
const initial: CountryActionResult = { success: false };

/**
 * One row per country. Collapsed shows flag + name + short code.
 *
 * In edit mode, clicking the row expands an inline edit form.
 * In read-only mode, the row is just a static info display — no button,
 * no expansion.
 */
function CountryRow({
  team,
  ...
  // Read-only: render a non-interactive info row. We still show flag, name,
  // short code, and flag code so the admin can see what's currently set.
  if (readOnly) {
    return (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            ...
            <div className="sm:col-span-2">
              ...
              <div className="flex items-center gap-2">
                ...
                />
                {/* Live preview of the current saved flag. Updates on save. */}
                <TeamFlag
                  flagCode={team.flag_code}
                  teamName={team.name}
                  shortCode={team.short_code}
